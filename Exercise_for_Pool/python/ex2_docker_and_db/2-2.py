...
def make_database_engine():
    database_url = DatabaseURL.create(
        drivername="mysql+pymysql",
        username=os.getenv("MYSQL_USER", "ex2_user"),
        password=os.getenv("MYSQL_PASSWORD", "ex2_password"),
        host=os.getenv("MYSQL_HOST", "db"),
        port=int(os.getenv("MYSQL_PORT", "3306")),
        database=os.getenv("MYSQL_DATABASE", "ex2"),
    )
    ...
    try:
        ...
        with engine.connect() as connection:
            ...
            url_count = connection.execute(
                text(f"SELECT COUNT(URL) FROM {TABLE_NAME}")
            ).scalar_one()
    finally:
        engine.dispose()

    print(f"{TABLE_NAME} テーブルに {total_count} 件保存しました")
    print(f"URL取得件数: {url_count} 件")
